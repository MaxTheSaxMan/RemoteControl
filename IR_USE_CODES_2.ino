//IR_USE_CODES_2
//1Max Woodhead, Jun 2025

/* 
1. You will need to use your own codes matching the buttons on your remote control.
These codes can be found by running the sketch IR_GET_CODES

2. Make sure that you use the 0x prefix to indicate that they are hexadecimal numbers.

3. You can assign any number of button codes in the loop and any suitable code for the button presses.

4.You might want to experiment with the delay.

5. Note that delay is blocking code and if it appears anywhere in the loop all processing will stop during the delay.
You will need to use the millis function to overcome this limitation.
*/

//Include the IRremote library and specify the receive pin
#include <IRremote.h>
#define receivePin 2

//Define pin variables
int redGreenPin = 10;
int bluePin = 9;
int greenPin = 8;
int redPin = 7;
int fadePin = 6;

//Define a brightness variable
int brightness = 0;

void setup() {
  //Initialise the receiver
  IrReceiver.begin(receivePin);

//Specify pins as inputs or outputs
  pinMode(receivePin, INPUT);
  pinMode(redPin, OUTPUT);
  pinMode(greenPin, OUTPUT);
  pinMode(bluePin, OUTPUT);
  pinMode(redGreenPin, OUTPUT);
  pinMode(fadePin, OUTPUT);

//Turn off LEDS
  digitalWrite(redPin, LOW);
  digitalWrite(greenPin, LOW);
  digitalWrite(bluePin, LOW);
  digitalWrite(redGreenPin, LOW);

  //One of the two LEDs on this pin will be on
  digitalWrite(fadePin, LOW);

  Serial.begin(9600);
  delay(1000);
}
void loop() {
  if (IrReceiver.decode()) {
    //Check which button has been pressed and execute the corresponding code
    switch (IrReceiver.decodedIRData.decodedRawData) {
      case 0xBA100AA0:  //Button 1
        ledToggle(redPin);
        break;
      case 0xBB110AA0: //Button 2
        ledToggle(greenPin);  
        break;
      case 0xB8120AA0:  //Button 3
        ledToggle(bluePin);
        break;
      case 0xB9130AA0:  //Button 4
        ledToggle(redGreenPin);
        Serial.println(digitalRead(redGreenPin));
        break;
      case 0xBE140AA0:  //Button 5
        brightness = brightness + 25;
        if (brightness > 255) {
          brightness = 255;
        }
        analogWrite(fadePin, brightness);
        break;
      case 0xBF150AA0:  //Button 6
        brightness = brightness - 25;
        if (brightness < 0) {
          brightness = 0;
        }
        analogWrite(fadePin, brightness);
        break;
    }
    delay(500);
  }
  IrReceiver.resume();
}

//Toggle function
void ledToggle(int ledPin) {
  digitalWrite(ledPin, !digitalRead(ledPin));
}
